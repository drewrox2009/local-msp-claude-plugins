---
description: >
  Use this skill when working with the RepairShopr API - authentication,
...
triggers:
  ...
  - repairshopr api key
  - repairshopr pagination
  - repairshopr rate limit
  - api error repairshopr
  - repairshopr rest api
  - repairshopr integration
---

# RepairShopr API Patterns

## Overview

The RepairShopr API provides access to tickets, customers, assets, invoices, and more. This skill covers authentication, pagination, rate limiting, error handling, and best practices for API integration.

## Authentication

### API Key Authentication

RepairShopr uses Bearer token authentication with API keys:
...
1. Log into your RepairShopr account
...
```bash
export REPAIRSHOPR_API_KEY="your-api-key-here"
export REPAIRSHOPR_SUBDOMAIN="your-subdomain"  # e.g., "acme" for acme.repairshopr.com
```
...
**Example:**
```
https://acme.repairshopr.com/api/v1/tickets
```
...
async function fetchAllTickets(filter = {}) {
  ...
  while (hasMore) {
    const response = await fetch(
      `https://${SUBDOMAIN}.repairshopr.com/api/v1/tickets?page=${page}`,
      {
        headers: {
...
RepairShopr enforces a rate limit of **180 requests per minute** per IP address.
...
```bash
# List tickets
curl -X GET "https://acme.repairshopr.com/api/v1/tickets?page=1" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json"

# Create ticket
curl -X POST "https://acme.repairshopr.com/api/v1/tickets" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "customer_id": 12345,
...
- [RepairShopr Tickets](../tickets/SKILL.md) - Ticket management
- [RepairShopr Customers](../customers/SKILL.md) - Customer management
- [RepairShopr Assets](../assets/SKILL.md) - Asset management
- [RepairShopr Invoices](../invoices/SKILL.md) - Invoice management

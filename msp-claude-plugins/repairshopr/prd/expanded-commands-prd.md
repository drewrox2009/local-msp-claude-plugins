# RepairShopr Expanded Commands PRD
...
This PRD defines 8 additional slash commands for the RepairShopr Claude Code plugin. These commands extend the existing `/create-ticket` and `/search-tickets` functionality to provide comprehensive ticket management, time tracking, alert handling, and customer operations support.
...
**Description:** Update fields on an existing RepairShopr ticket including status, priority, problem type, and assignment.
...
|-----------|------|----------|-------------|
| `ticket_id` | integer | Yes | The RepairShopr ticket ID to update |
...
Technicians need to update ticket status in real-time as work progresses. This command keeps them in their workflow without context-switching to the RepairShopr web UI.

---

### 2. /add-ticket-comment

**Description:** Add a comment to an existing RepairShopr ticket, with options for visibility and internal notes.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `ticket_id` | integer | Yes | The RepairShopr ticket ID |
...
**Description:** Log a time entry against a RepairShopr ticket for billing and tracking purposes.
...
All commands use the RepairShopr API with:
- API key passed as `api_key` query parameter
- Base URL: `https://{subdomain}.repairshopr.com/api/v1`

### Rate Limiting

RepairShopr enforces 180 requests per minute. Commands should:
...
RepairShopr uses page-based pagination:
...
- RepairShopr subscription with API access

## References

- [RepairShopr API Documentation](https://api-docs.repairshopr.com/)
- [RepairShopr Developer Resources](https://help.repairshopr.com/hc/en-us/articles/360023308074-API-Overview)

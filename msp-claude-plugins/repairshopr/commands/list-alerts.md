---
name: list-alerts
description: List active RMM alerts from RepairShopr
arguments:
  - name: customer_id
...
- RepairShopr RMM enabled on your account
...
2. **Fetch alerts**
   ```bash
   curl -s -X GET \
     "https://${REPAIRSHOPR_SUBDOMAIN}.repairshopr.com/api/v1/rmm_alerts?api_key=${REPAIRSHOPR_API_KEY}&status=active&customer_id=${customer_id}&per_page=${limit}"
   ```

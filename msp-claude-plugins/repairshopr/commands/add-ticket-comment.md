...
description: Add a comment to an existing RepairShopr ticket
arguments:
  - name: ticket_id
    description: The RepairShopr ticket ID
    required: true
  - name: comment
...
# Add RepairShopr Ticket Comment

Add a comment to an existing RepairShopr ticket, with options for visibility and email notifications.

## Prerequisites

- Valid RepairShopr API key configured
- Ticket must exist in RepairShopr
...
3. **Add the comment**
   ```bash
   curl -s -X POST \
     "https://${REPAIRSHOPR_SUBDOMAIN}.repairshopr.com/api/v1/tickets/${ticket_id}/comments?api_key=${REPAIRSHOPR_API_KEY}" \
...
URL: https://acme.repairshopr.com/tickets/12345
```
...
RepairShopr comments are limited to 65,535 characters.

---
name: update-ticket
description: Update fields on an existing RepairShopr ticket
arguments:
  - name: ticket_id
    description: The RepairShopr ticket ID to update
    required: true
  - name: status
...
# Update RepairShopr Ticket

Update fields on an existing RepairShopr ticket including status, priority, problem type, and assignment.
...
4. **Execute the update**
   ```bash
   curl -s -X PUT \
     "https://${REPAIRSHOPR_SUBDOMAIN}.repairshopr.com/api/v1/tickets/${ticket_id}?api_key=${REPAIRSHOPR_API_KEY}" \

---
name: resolve-alert
description: Resolve an RMM alert in RepairShopr
arguments:
  - name: alert_id
...
2. **Resolve the alert**
   ```bash
   curl -s -X PUT \
     "https://${REPAIRSHOPR_SUBDOMAIN}.repairshopr.com/api/v1/rmm_alerts/${alert_id}/resolve?api_key=${REPAIRSHOPR_API_KEY}" \
...
3. **Create ticket if requested**
   ...
   curl -s -X POST \
     "https://${REPAIRSHOPR_SUBDOMAIN}.repairshopr.com/api/v1/tickets?api_key=${REPAIRSHOPR_API_KEY}" \
     -H "Content-Type: application/json" \
     -d '{
...
Ticket Created:
  ...
  URL: https://acme.repairshopr.com/tickets/54321
```
...
Your RepairShopr account does not have RMM features enabled.

Contact RepairShopr support to enable RMM functionality.
```

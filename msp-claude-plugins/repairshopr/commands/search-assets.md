---
name: search-assets
description: Search for customer assets in RepairShopr
arguments:
  - name: query
...
2. **Execute search**
   ...
   # Search by query
   curl -s -X GET \
     "https://${REPAIRSHOPR_SUBDOMAIN}.repairshopr.com/api/v1/assets?api_key=${REPAIRSHOPR_API_KEY}&query=${query}&per_page=${limit}"

   # Or by customer
   curl -s -X GET \
     "https://${REPAIRSHOPR_SUBDOMAIN}.repairshopr.com/api/v1/customers/${customer_id}/assets?api_key=${REPAIRSHOPR_API_KEY}&per_page=${limit}"
   ```
...
Quick Actions:
- View asset: Click asset name in RepairShopr
...
URL: https://acme.repairshopr.com/assets/67890
```

---
name: log-time
description: Log a time entry against a RepairShopr ticket
arguments:
  - name: ticket_id
...
# Log Time to RepairShopr Ticket

Log a time entry against a RepairShopr ticket for billing and tracking purposes.
...
4. **Create time entry**
   ```bash
   curl -s -X POST \
     "https://${REPAIRSHOPR_SUBDOMAIN}.repairshopr.com/api/v1/tickets/${ticket_id}/timer_entries?api_key=${REPAIRSHOPR_API_KEY}" \
...
Use the RepairShopr web interface to find valid product IDs,

 ...
 * and error handling for the RepairShopr MCP server's HTTP transport mode.
 */
...
/**
 * Create a test HTTP server that mimics the repairshopr-mcp HTTP routing logic.
...
describe("HTTP Transport - gateway mode", () => {
  ...
  it("should return 401 when gateway credentials are missing", async () => {
    ...
    expect(body.required).toEqual(["X-RepairShopr-API-Key"]);
  });
  ...
  it("should accept requests with valid gateway headers", async () => {
    const { status, body } = await request(baseUrl, "/mcp", {
      ...
      headers: {
        ...
        "x-repairshopr-api-key": "test-api-key-123",
        "x-repairshopr-subdomain": "test-company",
      },
      body: JSON.stringify({}),
    });
    expect(status).toBe(200);
    ...
    expect(process.env.REPAIRSHOPR_API_KEY).toBe("test-api-key-123");
    expect(process.env.REPAIRSHOPR_SUBDOMAIN).toBe("test-company");
  });
  ...
  it("should accept requests with only the required API key header", async () => {
    const { status, body } = await request(baseUrl, "/mcp", {
      ...
      headers: {
        "Content-Type": "application/json",
        "x-repairshopr-api-key": "another-key",
      },
      body: JSON.stringify({}),
    });
    expect(status).toBe(200);
    expect(body.status).toBe("mcp-endpoint-reached");
    expect(process.env.REPAIRSHOPR_API_KEY).toBe("another-key");
  });
...
describe("applyGatewayCredentials", () => {
  ...
  it("should return true and set env when api key header is present", () => {
    const mockReq = {
      headers: { "x-repairshopr-api-key": "my-key" },
    } as unknown as IncomingMessage;
    expect(applyGatewayCredentials(mockReq)).toBe(true);
    expect(process.env.REPAIRSHOPR_API_KEY).toBe("my-key");
  });

  it("should set subdomain env when header is present", () => {
    const mockReq = {
      headers: {
        "x-repairshopr-api-key": "my-key",
        "x-repairshopr-subdomain": "my-company",
      },
    } as unknown as IncomingMessage;
    applyGatewayCredentials(mockReq);
    expect(process.env.REPAIRSHOPR_SUBDOMAIN).toBe("my-company");
  });
});

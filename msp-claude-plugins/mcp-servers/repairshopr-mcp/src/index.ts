#!/usr/bin/env node
/**
 * RepairShopr MCP Server with Decision Tree Architecture
 *
...
const navigateTool: Tool = {
  ...
  description:
    "Navigate to a RepairShopr domain to access its tools. Available domains: customers (manage customer accounts), tickets (manage support tickets), assets (manage configuration items/devices), contacts (manage customer contacts), invoices (view and manage billing).",
  inputSchema: {
    type: "object",
    properties: {
      domain: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
      ...
      if (!creds) {
        return {
          content: [
            {
              type: "text",
              text: "Error: No API credentials configured. Please set REPAIRSHOPR_API_KEY environment variable.",
            },
          ],
          isError: true,
        };
        ...
        content: [
          {
            ...
            text: `Navigated to ${domain} domain.\n\nAvailable tools:\n${domainTools
              ...
              .join("\n")}\n\nUse repairshopr_back to return to the main menu.`,
          },
        ],
      };
    }
    ...
    // Handle status
    if (name === "repairshopr_status") {
      const creds = getCredentials();
      const credStatus = creds
        ? `Configured${creds.subdomain ? ` (subdomain: ${creds.subdomain})` : ""}`
        : "NOT CONFIGURED - Please set REPAIRSHOPR_API_KEY environment variable";

      return {
        content: [
          {
            type: "text",
            text: `RepairShopr MCP Server Status\n\nCurrent domain: ${currentDomain || "(none - at main menu)"}\nCredentials: ${credStatus}\nAvailable domains: ${getAvailableDomains().join(", ")}\nRate limit: 180 requests/minute`,
          },
        ],
      };
    }
      ...
      content: [
        {
          type: "text",
          text: currentDomain
            ? `Unknown tool: ${name}. You are currently in the ${currentDomain} domain. Use repairshopr_back to return to the main menu.`
            : `Unknown tool: ${name}. Use repairshopr_navigate to select a domain first.`,
        },
      ],
      isError: true,
    };
...
function applyGatewayCredentials(req: IncomingMessage): boolean {
  ...
  const apiKey = getHeader("x-repairshopr-api-key");
  const subdomain = getHeader("x-repairshopr-subdomain");
  ...
  process.env.REPAIRSHOPR_API_KEY = apiKey;
  if (subdomain) {
    process.env.REPAIRSHOPR_SUBDOMAIN = subdomain;
  }
...
async function startHttpTransport(): Promise<void> {
        ...
        if (!hasCredentials) {
          ...
          res.end(
            JSON.stringify({
              ...
              message:
                "Gateway mode requires X-RepairShopr-API-Key header",
  ...
  await new Promise<void>((resolve) => {
    httpServer!.listen(port, host, () => {
      console.error(`RepairShopr MCP server listening on http://${host}:${port}/mcp`);
      console.error(
...
async function main() {
  ...
  if (transportType === "http") {
    ...
    await server.connect(transport);
    console.error("RepairShopr MCP server running on stdio (decision tree mode)");
  }
}
